...
class Config:
    # Dados
    train_images_dir = '/home/emanuel/Documentos/mestrado/treino dos modelos/imagens_mergeadas_train'
    train_masks_dir = '/home/emanuel/Documentos/mestrado/bases de dados/FIVES/train/Ground truth'
    test_images_dir = '/home/emanuel/Documentos/mestrado/treino dos modelos/imagens_mergeadas_test'
    test_masks_dir = '/home/emanuel/Documentos/mestrado/bases de dados/FIVES/test/Ground truth'
    
    # TAMANHOS DIFERENTES PARA IMAGEM E MÁSCARA
    img_height = 224   # Altura da imagem (panorâmica)
    img_width = 448    # Largura da imagem (2x a altura)
    mask_size = 224    # Máscara permanece 224x224 (quadrada)
    ...
    save_results = True
    results_dir = './results'
    model_name = 'ResNet101_UNet_Panoramic'
...
# ============================================
# MODELO AJUSTADO PARA IMAGENS PANORÂMICAS
# ============================================
class ResNet101UNetPanoramic(nn.Module):
    """
    U-Net com encoder ResNet101 para imagens panorâmicas (altura = largura/2)
    """
    def __init__(self, num_classes=1, pretrained=True):
        super(ResNet101UNetPanoramic, self).__init__()
...
# ============================================
# DATASET COM TAMANHOS DIFERENTES
# ============================================
class FundusSegmentationDatasetPanoramic(Dataset):
    """
    Dataset onde:
    - Imagens: 224x448 (panorâmica)
    - Máscaras: 224x224 (quadrada)
    """
    def __init__(self, images_dir, masks_dir, transform=None, mask_transform=None, 
                 img_height=224, img_width=448, mask_size=224):
        ...
        # Verificar quais imagens têm máscara correspondente
        self.valid_pairs = []
...
class DiceBCELoss(nn.Module):
    ...
    def forward(self, preds, targets):
        # Redimensionar preds para o tamanho da máscara (224x224)
...
def compute_metrics(preds, targets, threshold=0.5):
    ...
    preds: array numpy com shape (batch, 1, H, W) onde H=224, W=448
    targets: array numpy com shape (batch, 1, H, W) onde H=224, W=224
    """
    # Redimensionar preds para o tamanho da máscara (224x224)
    if preds.shape[2:] != targets.shape[2:]:
        # Redimensionar cada imagem do batch
        batch_size = preds.shape[0]
        preds_resized = np.zeros((batch_size, 1, targets.shape[2], targets.shape[3]), dtype=np.float32)
        
        for i in range(batch_size):
            preds_resized[i, 0] = cv2.resize(
                preds[i, 0], 
                (targets.shape[3], targets.shape[2]), 
    ...
    preds_sum = preds_binary.sum()
    targets_sum = targets_binary.sum()
    
    dice = (2.0 * intersection) / (preds_sum + targets_sum + 1e-6)
    
    union = np.logical_or(preds_binary, targets_binary).sum()
    iou = intersection / (union + 1e-6)
    
    sens = intersection / (targets_sum + 1e-6)
    
    tn = np.logical_and(1 - preds_binary, 1 - targets_binary).sum()
...
def train_model(model, train_loader, val_loader, config, run_id=0):
    ...
    print(f"Imagem: {config.img_height}x{config.img_width} (panorâmica)")
    print(f"Máscara: {config.mask_size}x{config.mask_size} (quadrada)")
...
def plot_training_history(history, run_id, config):
    ...
    fig.suptitle(f'{config.model_name} - Run {run_id+1}\nImagem: {config.img_height}x{config.img_width} | Máscara: {config.mask_size}x{config.mask_size}', 
...
def create_summary_report(df_final, df_best, stats, config):
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"📐 IMAGEM: {config.img_height}x{config.img_width} (Panorâmica)\n")
        f.write(f"📐 MÁSCARA: {config.mask_size}x{config.mask_size} (Quadrado)\n")
        f.write(f"📊 NÚMERO DE EXECUÇÕES: {config.n_runs}\n")
        f.write(f"🔄 ÉPOCAS MÁXIMAS: {config.epochs}\n")
        f.write(f"⏳ PACIÊNCIA: {config.patience} épocas\n")
...
def test_model_average(config):
    ...
    test_dataset = FundusSegmentationDatasetPanoramic(
    ...
    for run_id in range(config.n_runs):
        ...
        model = ResNet101UNetPanoramic(num_classes=1)
    ...
    if all_test_metrics:
        ...
        print(f"Imagem: {config.img_height}x{config.img_width} | Máscara: {config.mask_size}x{config.mask_size}")
        print("="*50)
...
def main():
    ...
    print("\n📂 Carregando dados...")
    try:
        full_dataset = FundusSegmentationDatasetPanoramic(
    ...
    for run_id in range(config.n_runs):
        ...
        # Criar modelo
        try:
            model = ResNet101UNetPanoramic(num_classes=1, pretrained=True)
            model = model.to(config.device)
        except Exception as e:
