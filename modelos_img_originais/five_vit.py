...
# -*- coding: utf-8 -*-
"""
Segmentação de Vasos em Fundoscopia - ViTUNet (RGB)
...
class Config:
    def __init__(self, args):
        ...
        self.best_model_path = './best_ViTUNet_segmentation.pth'
        ...
        self.input_channels = 3  # RGB colorido
...
class ViTUNet(nn.Module):
    ...
    def forward(self, x):
        # Garantir 3 canais
        if x.shape[1] != 3:
            raise ValueError(f"Esperado 3 canais, recebeu {x.shape[1]}")
        ...
        x = x[:, 1:, :]
        x = x.transpose(1, 2).view(batch_size, 768, 14, 14)
        ...
        if d4.shape[2] != 224 or d4.shape[3] != 224:
            d4 = nn.functional.interpolate(d4, size=(224, 224), mode='bilinear', align_corners=False)
...
def inspect_vit():
    ...
    if hasattr(vit, 'pos_embedding'):
        ...
        print("  Nenhum atributo 'pos_embedding' ou 'pos_embed' encontrado!")
...
class FundusSegmentationDataset(Dataset):
    def __init__(self, images_dir, masks_dir, img_size=224):
        ...
        print(f"Imagens: {len(self.images_paths)}, Mascaras: {len(self.masks_paths)}")
            ...
            if name in mask_names:
                ...
                for mname, mpath in mask_names.items():
                    if os.path.splitext(mname)[0] == base:
                        ...
                        break
        print(f"Pares validos: {len(self.valid_pairs)}")
...
def train_model(model, train_loader, val_loader, config, run_id=0):
    ...
    print(f"Paciencia: {config.patience}, Epocas: {config.epochs}")
    ...
    for epoch in range(config.epochs):
        print(f"\n{'='*50}\nRun {run_id+1} - Epoca {epoch+1}/{config.epochs} | LR: {optimizer.param_groups[0]['lr']:.6f}")
        ...
        if patience_counter >= config.patience:
            ...
            print(f"\nEarly stopping na epoca {stopped_epoch}")
            break
...
# ============================================
# FUNÇÃO DE TESTE DETALHADA PARA MÚLTIPLAS BASES
# ============================================
def test_model_detailed(model, test_loader, device, run_id, config, dataset_name="test"):
    ...
    if config.save_results:
        # Criar diretório específico para este dataset
...
def test_model_average_detailed(config):
    """
    Testa todos os modelos treinados em todas as bases de dados configuradas
    """
    all_results = {}
    ...
    for dataset_info in config.test_datasets:
        ...
        if all_per_image_dfs:
            # Salvar resultados consolidados para este dataset
            dataset_test_dir = os.path.join(config.test_results_dir, dataset_name)
            os.makedirs(dataset_test_dir, exist_ok=True)
...
def visualize_results(config):
    ...
    # Tentar carregar todos os summaries
    all_summaries = []
    for run_id in range(config.n_runs):
...
# ============================================
def parse_args():
    parser = argparse.ArgumentParser(description='ViTUNet para Segmentacao de Vasos em Fundoscopia')
    ...
    parser.add_argument('--batch_size', type=int, default=8, help='Batch size (padrao: 8)')
    parser.add_argument('--epochs', type=int, default=50, help='Numero de epocas (padrao: 50)')
    parser.add_argument('--learning_rate', type=float, default=0.0001, help='Learning rate (padrao: 0.0001)')
    ...
    parser.add_argument('--num_workers', type=int, default=2, help='Numero de workers (padrao: 2)')
    ...
    parser.add_argument('--model_name', type=str, default='ViTUNet', help='Nome do modelo (padrao: ViTUNet)')
...
def main():
    ...
    print(f"{' ' * 15}Segmentacao de Vasos em Fundoscopia (RGB)")
    print("="*70)
    ...
    print(f"  Validacao: {len(val_dataset)} imagens (RGB)")
