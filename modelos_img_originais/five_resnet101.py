...
# -*- coding: utf-8 -*-
"""
Segmentação de Vasos em Fundoscopia - ResNet101 UNet (RGB)
...
class Config:
    def __init__(self, args):
        ...
        self.best_model_path = './best_resnet101_segmentation.pth'
...
class ResNet101UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Ajustar tamanho se necessário para ResNet
        ...
        if d1.shape[2] != 224 or d1.shape[3] != 224:
...
# ============================================
def parse_args():
    parser = argparse.ArgumentParser(description='ResNet101 UNet para Segmentacao de Vasos em Fundoscopia')
    ...
    parser.add_argument('--model_name', type=str, default='ResNet101_UNet', help='Nome do modelo (padrao: ResNet101_UNet)')
...
def main():
    ...
    for run_id in range(config.n_runs):
        ...
        try:
            model = ResNet101UNet(num_classes=config.num_classes, pretrained=config.pretrained)
            model = model.to(config.device)
