...
class Config:
    ...
    # Diretórios
    checkpoint_dir = './checkpoints_SwinUNet'
    best_model_path = './best_SwinUNet_segmentation.pth'
    logs_dir = './logs_SwinUNet'
...
# ============================================
# MODELO Swin Transformer UNet - SOLUÇÃO DEFINITIVA
# ============================================
class SwinUNet(nn.Module):
    """
    U-Net com encoder Swin Transformer - SOLUÇÃO QUE FUNCIONA
    """
    def __init__(self, num_classes=1, pretrained=True):
        super(SwinUNet, self).__init__()
        
        # Carregar Swin Transformer Tiny
        if pretrained:
            try:
        ...
        # CORREÇÃO: Criar um wrapper que converte o formato da entrada
        # O Swin espera (B, H, W, C) mas vamos converter internamente
        
        # Extrair os componentes do Swin
        self.patch_embed = self.swin.features[0]  # PatchEmbed - espera (B, H, W, C)
        self.layer1 = self.swin.features[1]       # 96 canais, 56x56
        self.layer2 = self.swin.features[2]       # 192 canais, 28x28
        self.layer3 = self.swin.features[3]       # 384 canais, 14x14
        self.layer4 = self.swin.features[4]       # 768 canais, 7x7
        ...
        self.dec4 = nn.Sequential(
            nn.Conv2d(256 + 384, 256, 3, padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
        ...
        self.dec3 = nn.Sequential(
            nn.Conv2d(128 + 192, 128, 3, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
        ...
        self.dec2 = nn.Sequential(
            nn.Conv2d(64 + 96, 64, 3, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
        ...
        self.dec1 = nn.Sequential(
            nn.Conv2d(32 + 96, 32, 3, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        # CORREÇÃO: Converter para o formato que o Swin espera
        # Swin espera (B, H, W, C)
        x = x.permute(0, 2, 3, 1)  # (B, H, W, C)
        
        # Agora passa pelo Swin
        # PatchEmbed: (B, H, W, C) -> (B, C, H, W) (internamente o PatchEmbed faz a conversão)
        e1 = self.patch_embed(x)  # (B, 96, 56, 56)
        e1 = self.layer1(e1)      # (B, 96, 56, 56)
        
        e2 = self.layer2(e1)      # (B, 192, 28, 28)
        e3 = self.layer3(e2)      # (B, 384, 14, 14)
        e4 = self.layer4(e3)      # (B, 768, 7, 7)
        
        # Center
        center = self.center(e4)  # (B, 512, 7, 7)
        
        # Decoder com skip connections
        # Nível 4: 7x7 -> 14x14
        d4 = self.up4(center)  # (B, 256, 14, 14)
        if d4.shape[2:] != e3.shape[2:]:
            d4 = nn.functional.interpolate(d4, size=e3.shape[2:], mode='bilinear', align_corners=False)
        d4 = torch.cat([d4, e3], dim=1)  # (B, 640, 14, 14)
        ...
        d3 = torch.cat([d3, e2], dim=1)  # (B, 320, 28, 28)
        ...
        d2 = torch.cat([d2, e1], dim=1)  # (B, 160, 56, 56)
        d2 = self.dec2(d2)  # (B, 64, 56, 56)
        
        # Nível 1: 56x56 -> 112x112
        d1 = self.up1(d2)  # (B, 32, 112, 112)
...
class FundusSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Converter para tensor (channel-first para PyTorch)
...
def plot_training_history(history):
    ...
    ax1.set_ylabel('Loss')
    ax1.set_title('Loss Curves - SwinUNet')
    ax1.legend()
    ax1.grid(True)
    ...
    ax2.set_title('Dice Score Curves - SwinUNet')
    ax2.legend()
    ax2.grid(True)
    
    plt.tight_layout()
    plt.savefig('training_history_SwinUNet.png', dpi=300)
    plt.show()
...
# ============================================
def main():
    print("🚀 Treinamento Swin Transformer UNet para Segmentação de Vasos em Fundoscopia")
    print(f"Device: {config.device}")
    ...
    # 2. Criar modelo
    print("\n🏗️ Construindo modelo Swin Transformer UNet para segmentação...")
