...
# -*- coding: utf-8 -*-
"""
Segmentação de Vasos em Fundoscopia - ViTUNet (Grayscale)
...
class Config:
    def __init__(self, args):
        ...
        self.best_model_path = './best_ViTUNet_grayscale.pth'
...
# ============================================
# MODELO ViTUNet para GRAYSCALE
# ============================================
class ViTUNet(nn.Module):
    def __init__(self, num_classes=1, pretrained=True, img_size=224, input_channels=1):
        ...
        # Modificar a primeira convolução para aceitar 1 canal
...
def test_model_average_detailed(config):
    ...
    for dataset_info in config.test_datasets:
        ...
        for run_id in range(config.n_runs):
            ...
            model = ViTUNet(num_classes=1, pretrained=False, input_channels=input_channels)
            model_path = os.path.join(config.models_dir, f'best_model_run_{run_id}.pth')
...
# ============================================
def parse_args():
    parser = argparse.ArgumentParser(description='ViTUNet para Segmentacao de Vasos em Fundoscopia (Grayscale)')
    ...
    parser.add_argument('--model_name', type=str, default='ViTUNet_Grayscale', help='Nome do modelo (padrao: ViTUNet_Grayscale)')
...
def main():
    ...
    if all_best_dices:
        ...
        print(f"\nMedia: {best_dice_array.mean():.4f} ± {best_dice_array.std():.4f}")
