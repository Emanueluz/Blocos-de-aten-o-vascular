...
class Config:
    ...
    # Diretórios
    checkpoint_dir = './checkpoints_MobileNetV2UNet'
    best_model_path = './best_MobileNetV2UNet_segmentation.pth'
    logs_dir = './logs_MobileNetV2UNet'
...
# ============================================
# MODELO MobileNetV2UNet CORRIGIDO
# ============================================
class MobileNetV2UNet(nn.Module):
    """
    U-Net com encoder MobileNetV2 - VERSÃO CORRIGIDA
    """
    def __init__(self, num_classes=1, pretrained=True):
        ...
        # MobileNetV2 channels reais:
        # features[0]: Conv2d(3, 32) -> 32 canais
        # features[1]: InvertedResidual(32, 16) -> 16 canais
        # features[2]: InvertedResidual(16, 24) -> 24 canais
        # features[3]: InvertedResidual(24, 24) -> 24 canais
        # features[4]: InvertedResidual(24, 32) -> 32 canais
        # features[5]: InvertedResidual(32, 32) -> 32 canais
        # features[6]: InvertedResidual(32, 32) -> 32 canais
        # features[7]: InvertedResidual(32, 64) -> 64 canais
        # features[8-13]: InvertedResidual(64, 96) -> 96 canais
        # features[14]: InvertedResidual(96, 160) -> 160 canais
        # features[15]: InvertedResidual(160, 160) -> 160 canais
        # features[16]: InvertedResidual(160, 160) -> 160 canais
        # features[17]: InvertedResidual(160, 320) -> 320 canais
        
        # Encoder - com canais corretos
        self.enc1 = features[:1]      # 32 canais, 112x112
        self.enc2 = features[1:3]     # 24 canais, 56x56
        self.enc3 = features[3:6]     # 32 canais, 28x28
        self.enc4 = features[6:13]    # 96 canais, 14x14
        self.enc5 = features[13:18]   # 320 canais, 7x7
        ...
        # Decoder - CORRIGIDO com canais corretos
        # 7x7 -> 14x14
        self.up5 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        self.dec5 = nn.Sequential(
            nn.Conv2d(128 + 96, 128, 3, padding=1),  # 224 canais
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
        ...
        self.dec4 = nn.Sequential(
            nn.Conv2d(64 + 32, 64, 3, padding=1),  # 96 canais
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
        ...
        self.dec3 = nn.Sequential(
            nn.Conv2d(32 + 24, 32, 3, padding=1),  # 56 canais
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        e2 = self.enc2(e1)  # 24 x 56 x 56
        e3 = self.enc3(e2)  # 32 x 28 x 28
        e4 = self.enc4(e3)  # 96 x 14 x 14
        ...
        d5 = torch.cat([d5, e4], dim=1)  # 128 + 96 = 224
        ...
        d4 = torch.cat([d4, e3], dim=1)  # 64 + 32 = 96
        ...
        d3 = torch.cat([d3, e2], dim=1)  # 32 + 24 = 56
...
# ============================================
def main():
    print("🚀 Treinamento MobileNetV2UNet para Segmentação de Vasos em Fundoscopia")
    print(f"Device: {config.device}")
    ...
    # 2. Criar modelo
    print("\n🏗️ Construindo modelo MobileNetV2UNet para segmentação...")
    model = MobileNetV2UNet(num_classes=config.num_classes, pretrained=config.pretrained)
